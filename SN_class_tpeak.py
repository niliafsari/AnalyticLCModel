...
class supernova:
    ...
    def tailNickel(self):
        ...
        if self.verbose:
            ...
            plt.gca().xaxis.set_minor_locator(AutoMinorLocator(5))
            plt.title(self.name)
            #plt.savefig('./Plots/'+self.name+'_Lbol_.pdf')
            plt.show()
        ...
        print "this tpeak for tail:",t_u_t[(t_u_t >= 60) & (t_u_t < 120)]+5
        for k,offset in enumerate(s):
            ...
            x, x_err = fit_bootstrap([0.2, 3], t_u_t[(t_u_t >= 60) & (t_u_t < 120)]+offset+5, lbol_t[(t_u_t >= 60) & (t_u_t < 120)],
                                     np.clip(le_t[(t_u_t >= 60) & (t_u_t < 120)], a_min=0.0001e42, a_max=1e51),wygodaErr,
    ...
    def khatami_model(self):
        from SNAP5.Analysis.LCFitting import fit_bootstrap
        beta0 = 0.8
        print "hey this tpeak for beta:",self.peakt[0]+5
        betafit0 = fit_bootstrap([beta0], [self.peakt[0]+5, self.tail_ni_mass[0]],[10**self.peakL[0]],[10], khatami_err,bounds=([0.0], [np.inf]),
                                 errfunc=True, perturb=False, n=1000, nproc=4)
        betafit1 = fit_bootstrap([beta0], [self.peakt[0]+5, self.tail_ni_mass[0]],[10**self.peakL[0]+10**self.peakL[1]],[10], khatami_err,bounds=([0.0], [np.inf]),
                                 errfunc=True, perturb=False, n=1000, nproc=4)
        betafit2 = fit_bootstrap([beta0], [self.peakt[0]+5, self.tail_ni_mass[0]],[10**self.peakL[0]-10**self.peakL[1]],[10], khatami_err,bounds=([0.0], [np.inf]),
                                 errfunc=True, perturb=False, n=1000, nproc=4)
...
for i,sn_name in enumerate(info[1:,0]):#info[1:,0]):#
    if  (i>27)  :
         continue
    print sn_name
    ...
    print sn.beta_req
    sn.prentice_data()
    with open('Data/'+sn.name+'_results_wygoda_tpeak.csv', 'w') as f:
