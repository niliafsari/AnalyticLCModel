...
plt.scatter([float(integer.split(';')[0]) for integer in Nilou_new['peakL'].tolist()],[np.log10(float(integer)*10**41) for integer in Meza_new['L_p'].tolist()] )
...
print np.mean(np.array([float(integer.split(';')[0]) for integer in Nilou_new['arnett_ni_mass'].tolist()]))
print np.mean(np.array([float(integer) for integer in Meza_new['Arnett Nickel'].tolist()]))
