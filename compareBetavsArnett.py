...
df_save = pd.DataFrame(columns=['name', 'type','Mni','Mni_e', 'Arnett','Tail'])
for index,sn in enumerate(df['name'].tolist()):
    ...
    df_save.loc[index] = [sn,df[df['name'] == sn]['sn_type_full'].tolist()[0],Mni,mni_e,float(df[df['name'] == sn]['arnett_ni_mass'].tolist()[0].split(";")[0]),float(df[df['name'] == sn]['tail_ni_mass'].tolist()[0].split(";")[0])]
print np.mean(df_save.iloc[:,2])
print np.sqrt(np.sum((df_save['Mni']-df_save['Tail'])**2))/27, np.sqrt(np.sum((df_save['Arnett']-df_save['Tail'])**2))/27
df_save.to_csv('./Data/khatamiMni.csv',index=False)