...
add=[]
for i,sn_name in enumerate(['SN1993J']):
    print(sn_name)
    print "name",sn_name
    ...
    plt.scatter(mag[:,0][mag[:,5]=="I"].astype(float)-t0,mag[:,3][mag[:,5]=="I"].astype(float))
    #plt.scatter(mag[:,0][mag[:,5]=="B"].astype(float)-t0,mag[:,3][mag[:,5]=="B"].astype(float))

    t_u = np.arange(10, 150, 0.1)
    M_u=np.zeros(shape=(len(Band)+2,np.shape(t_u)[0]))
    for i,band in enumerate(Band):
        t=mag[(mag[:,3]<19) & (mag[:, 5] == band), 0].astype(float)
        M=mag[(mag[:,3]<19) & (mag[:, 5] == band),3].astype(float)
        M = M[np.argsort(t)]
        t = t[np.argsort(t)]-t0
        t,u=np.unique(t,return_index=True)
        fit = UnivariateSpline(t,M[u], s=0.35)
        M_u[i,:]=fit(t_u)
    ...
    BC_V_I=0.213-0.203*(M_u[1, :] - M_u[0, :])-0.079*(M_u[1, :] - M_u[0, :])**2
